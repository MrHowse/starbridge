...
## 2026-02-20 — Debrief data passed via localStorage, not WebSocket (v0.03n)

**Decision**: Post-game debrief payload is computed server-side and included in the `game.over` stats dict. The client saves it to `localStorage('starbridge_debrief')`. A separate standalone page `/client/debrief/` reads localStorage on load and renders the analysis.

**Reasoning**: No additional message types, no REST endpoint, no re-broadcast. The `game.over` message is already sent to every station; adding `stats.debrief` piggybacks on an existing flow. The debrief page opens in a new tab (via "VIEW DEBRIEF" button) — it only needs the one saved payload, not a live WebSocket. localStorage is ephemeral per-browser-tab session, which is the right lifetime for post-game analysis.

**Alternatives considered**:
- REST endpoint `GET /debrief/latest` — rejected: requires the server to hold state after the game ends; server is stateless after `game.over` + `stop()`
- Re-broadcast debrief on reconnect — rejected: unnecessary; the data is already in the `game.over` payload
- sessionStorage instead of localStorage — considered: sessionStorage doesn't survive new tabs (required for debrief page opened via `target="_blank"`); localStorage is correct here

---

## 2026-02-20 — GameLogger._last_log_file preserves path after stop() (v0.03n)

**Decision**: `GameLogger.stop()` sets `self._last_log_file = self._log_file` before clearing `_log_file`. The `get_log_path()` method returns `self._log_file or self._last_log_file` — so callers get the active path during logging, or the last-used path after `stop()`.

**Reasoning**: The debrief must be computed from the complete log file (including the "session/ended" record written in `stop_logging()`). So the sequence in the game loop is: capture `_log_path = gl.get_log_path()` → call `gl.stop_logging()` → call `gdb.compute_from_log(_log_path)`. Without `_last_log_file`, `get_log_path()` returns `None` after `stop_logging()` clears `_log_file`, breaking the debrief computation.

**Alternatives considered**:
- Pass log path as a return value from `stop_logging()` — considered: would work but changes the stop() signature; the internal field approach is less invasive

---

## 2026-02-20 — Ship classes extended with min_crew and max_crew (v0.03o)

**Decision**: `ShipClass` Pydantic model gains `min_crew: int = 1` and `max_crew: int = 12` with defaults. All 7 ship JSON files define explicit crew ranges. Lobby payload includes `min_crew` and `max_crew`. The server does NOT enforce `min_crew` at launch (host can start with fewer players).

**Reasoning**: Backward-compatible defaults mean existing tests pass unmodified. Lobby payload gives the host UI the data to display crew requirements. Enforcement is deferred: for a LAN party with fewer players than the minimum, they should still be able to play (some stations unmanned). Hard enforcement is a v0.04 UX decision, not a v0.03 data model decision.

**Alternatives considered**:
- Enforce min_crew at launch → reject if below minimum — deferred: creates UX friction for small groups; identified as a known gap at v0.03 close

---

## 2026-02-20 — 7 ship classes: 5 combat + 2 specialised (v0.03o)

**Decision**: `SHIP_CLASS_ORDER` is extended to 7 entries: `["scout", "corvette", "frigate", "cruiser", "battleship", "medical_ship", "carrier"]`. The two specialised classes have differentiated hull/ammo values but no unique gameplay mechanics in v0.03. Their specialist features (medbay expansion, fighter complement) are explicitly deferred to v0.04.

**Reasoning**: Adding the classes now locks in their identity and data models (hull, ammo, crew ranges) before gameplay differentiation is designed. It also gives the lobby ship-select UI 7 distinct options and gives the test suite hooks to verify the class framework. Shipping half-implemented mechanics would be worse than shipping placeholder mechanics with accurate stat profiles.

**Alternatives considered**:
- Hold specialised classes until v0.04 mechanics are ready — rejected: the class data model is stable; deferring the JSON files would require a second pass on lobby + ship_class tests
- Give medical_ship/carrier identical stats to frigate — rejected: player-visible stat differentiation (hull, ammo) should already reflect the ship's identity even before unique mechanics land

---

## 2026-02-20 — Training missions: is_training flag + training_flag trigger type (v0.03m)

**Decision**: Training missions carry `"is_training": true` and a `"target_role"` string. A new trigger type `"training_flag"` fires when `MissionEngine.set_training_flag(flag_name)` is called by the relevant game loop module. Training flags are lightweight named events (e.g. `"helm_heading_set"`, `"weapons_beam_fired"`) set by station handlers when the target action occurs.

**Reasoning**: Training missions need to reward player actions that already happen in normal play — heading changes, beam fires, scans — without those actions triggering training objectives in non-training missions. The `is_training` flag gates the flag-setting code paths so game loop handlers only call `set_training_flag()` when a training session is active. The `training_flag` trigger is simpler than adding dedicated training-mode branches to the full mission engine trigger evaluator.

**Alternatives considered**:
- Repurpose existing triggers (timer_elapsed for all training steps) — rejected: timers don't verify the player actually performed the action; training should reward the action, not the wait
- A dedicated training engine (separate from MissionEngine) — rejected: code duplication; MissionEngine already handles objective sequencing; a new trigger type is the minimal extension

---

## 2026-02-20 — Difficulty presets as typed dataclass in server/difficulty.py (v0.03)

**Decision**: Four named difficulty presets (`cadet`, `officer`, `commander`, `admiral`) are defined as a `DifficultyPreset` dataclass with `enemy_damage_mult`, `puzzle_time_mult`, `hints_enabled`, and `spawn_rate_mult` fields. `get_preset(name)` falls back to `officer` for unknown names. Lobby includes selected preset in `game.started`.

**Reasoning**: A dataclass with named fields is more readable and testable than a dict of magic numbers. Fallback-to-officer means the system is resilient to typos or future preset renames. Four named presets cover the range from casual (cadet) to hardcore (admiral) without overwhelming the lobby UI.

**Alternatives considered**:
- Inline constants per-module (enemy damage mult in combat.py, puzzle time mult in game_loop.py) — rejected: impossible to change the difficulty without touching 4+ files
- User-editable config file — deferred: appropriate for v0.04 when persistent player profiles are in scope
