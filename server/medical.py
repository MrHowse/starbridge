"""
Medical Station Handler.

Receives medical.* messages from the Medical client, validates them against
...
logger = logging.getLogger("starbridge.medical")
...
async def handle_medical_message(connection_id: str, message: Message) -> None:
    """Validate and queue a Medical control message for the game loop."""
    ...
    try:
        ...
        logger.warning("Medical validation error from %s: %s", connection_id, exc)
        return

    if payload is None:
        logger.warning(
            "Unhandled medical message type '%s' from %s", message.type, connection_id
        )
        return
