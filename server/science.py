"""
Science Station Handler.

Receives science.* messages from the Science client, validates them against
...
logger = logging.getLogger("starbridge.science")


# ---------------------------------------------------------------------------
# Sender protocol — same decoupling pattern as engineering.py / weapons.py
# ---------------------------------------------------------------------------
...
async def handle_science_message(connection_id: str, message: Message) -> None:
    """Validate and queue a Science control message for the game loop."""
    ...
    try:
        ...
        logger.warning("Science validation error from %s: %s", connection_id, exc)
        return

    if payload is None:
        logger.warning(
            "Unhandled science message type '%s' from %s", message.type, connection_id
        )
        return
