"""
Comms Station Handler.

Receives comms.* messages from the Comms client, validates them against
...
logger = logging.getLogger("starbridge.comms")
...
async def handle_comms_message(connection_id: str, message: Message) -> None:
    """Validate and queue a Comms control message for the game loop."""
    ...
    try:
        ...
        logger.warning("Comms validation error from %s: %s", connection_id, exc)
        return

    if payload is None:
        logger.warning(
            "Unhandled comms message type '%s' from %s", message.type, connection_id
        )
        return
